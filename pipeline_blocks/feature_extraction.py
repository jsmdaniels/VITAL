...
def extract_features(signal, label, fe_method, **fe_parameters):
    ...
    pred_horizon      = pred_horizon_mins * 60 * 100
    ...
    if fe_method == 'RAW':
        ...
        for i in range(0, len(signal) - pred_horizon, fft_length):
        ...
        for i in range(0, len(signal), stft_length):
            ...
            if len(signal_slice) == stft_length:
                ...
                frequency_range, time_range, stft = scipy.signal.stft(x=signal_slice,
                                                                     ...
                                                                     nperseg=fe_parameters['stft_window_length'])#, noverlap=0)
...
def extract_features_segmented_class(signal, labels_df, label_name, pt_name, fe_method, **fe_parameters):
    ...
    if fe_method == 'RAW':
        ...
        stft_length = fe_parameters['num_stft_windows'] * fe_parameters['stft_window_length']
        num_stfts = (len(signal) - pred_horizon) / stft_length
        print(f'Number of STFTs for this patient: {int(np.floor(num_stfts)) : >13}\n')

        # - pred_horizon   | + pred_horizon
        if(int(np.floor(num_stfts)) >= 6):
            for i in range(0, len(signal) - pred_horizon, stft_length):
                signal_slice = signal[i:i + stft_length]
                ...
                if len(signal_slice) == stft_length:
                    ...
                    frequency_range, time_range, stft = scipy.signal.stft(x=signal_slice,
                                                                        fs=fe_parameters['sampling_rate'],
                                                                        nperseg=int(fe_parameters['stft_window_length']/fe_parameters['stft_resolution']))#, noverlap=0)
                                                                        

                    # add an extra dimension (2D -> 3D) since keras CNN model layers 
                    # require input format (batch_size, height, width, channels)
                    # require input format (batch_size, timesteps, height, width, channels)
                    stft = np.expand_dims(stft, axis=2)
                    
                    selected_label = labels_df.loc[(labels_df['start_idx'] <= i + pred_horizon ) & (labels_df['end_idx'] >= i + pred_horizon + stft_length)]
                    # selected_label_aux = labels_df.loc[(labels_df['start_idx'] <= i + pred_horizon ) & (labels_df['end_idx'] >= i + pred_horizon + stft_length)]
                    if not selected_label.empty:
                        # print(selected_label['shock_event'].values)
                        feature_label_pairs.append((stft, selected_label[label_name].values[0],selected_label['shock_event'].values[0] , pt_name))

            feature_label_pairs = np.asarray(feature_label_pairs, dtype=object)

        if fe_parameters['plot_spectrograms']:
            print("Plotting spectrogram "+str(num_stfts)+" "+str(frequency_range[-1])+" "+str(time_range[-1]))
            plot_spectrograms(feature_label_pairs, num_stfts, frequency_range, time_range)

    else:
        raise ValueError("Invalid choice of feature extraction method. Choose between ['RAW', 'FFT', 'STFT']")

    return feature_label_pairs

def extract_temporal_features_segmented_class(signal, labels_df, label_name, pt_name, fe_method, **fe_parameters):
    
    feature_label_pairs = []
    timesteps         = 5
    pred_horizon_mins = 120 #240 #360
    pred_horizon      = pred_horizon_mins * 60 * 100
    ...
    if fe_method == 'RAW':
        ...
        stft_length = fe_parameters['num_stft_windows'] * fe_parameters['stft_window_length']
        seq_len = timesteps*stft_length
        num_stfts = (len(signal) - pred_horizon - seq_len) / stft_length
        print(f'Number of STFTs for this patient: {int(np.floor(num_stfts)) : >13}\n')

        # - pred_horizon   | + pred_horizon
        if(int(np.floor(num_stfts)) >= 6):
            for i in range(seq_len, len(signal) - pred_horizon, stft_length):
                for k in range(0, timesteps):

                    m = i - (timesteps - k)*stft_length
                    n = i - (timesteps - (k + 1))*stft_length
                    
                    signal_slice = signal[m : n]
                    ...
                    if len(signal_slice) == stft_length:
                        ...
                        frequency_range, time_range, stft_ = scipy.signal.stft(x=signal_slice,
                                                                            fs=fe_parameters['sampling_rate'],
                                                                            nperseg=int(fe_parameters['stft_window_length']/fe_parameters['stft_resolution']))#, noverlap=0)
                        # stft_ = np.random.rand(4,7)                                                  

                        # add an extra dimension (2D -> 3D) since keras CNN model layers 
                        # require input format (batch_size, height, width, channels)
                        # require input format (batch_size, timesteps, height, width, channels)
                        stft_ = np.expand_dims(stft_, axis=0)
                        stft_ = np.expand_dims(stft_, axis=3)
                        if(k==0):
                            stft = stft_
                        else:
                            stft = np.append(stft, stft_, axis=0)
                    
                selected_label = labels_df.loc[(labels_df['start_idx'] <= i + pred_horizon) & (labels_df['end_idx'] >= i + pred_horizon + stft_length)]
                
                if not selected_label.empty:
                    #print(selected_label[label_name].values[0])
                    feature_label_pairs.append((stft, selected_label[label_name].values[0], pt_name))
...
def feature_extraction(base_path, event_type, reduced_patient_matrix, window_size_samples, filter_params, signal_wavelength, fe_method, segment_length, window_divider, num_stft_windows=1, stft_resolution=1, segmented_labels={}, label_name='clinical_score', plot_spectogram=False):
    model_data = []
    use_temporal_setting = False
    ...
    if fe_method == 'RAW':
        ...
        stft_window_length = get_stft_window_length(base_path, event_type, reduced_patient_matrix, filter_params, signal_wavelength, window_divider)
        fe_parameters = {
            'sampling_rate': filter_params['sample_rate_Hz'], 
            'stft_window_length': stft_window_length,
            'num_stft_windows': num_stft_windows, 
            'plot_spectrograms': plot_spectogram,
    ...
    for row in reduced_patient_matrix:
        ...
        if segmented_labels == {}:
            ...
            pt = patient.split('-')
            if use_temporal_setting:
                try:
                    labels_df = segmented_labels[pt[1]+"-"+pt[2]][recording_file_name]
                    stft_length = fe_parameters['num_stft_windows'] * fe_parameters['stft_window_length']
                    num_stfts = len(filtered_red_adc) / stft_length
                    print(f'Number of STFTs for this patient: {int(np.floor(num_stfts)) : >13}\n')
                    feature_label_pairs = extract_temporal_features_segmented_class(signal=filtered_red_adc, 
                                                        labels_df=labels_df,
                    ...
                    print("File "+recording_file_name+" for patient "+patient+" is outside of vital sign range\n"+str(e))
                    feature_label_pairs = []
            else:
                try:
                    labels_df = segmented_labels[pt[1]+"-"+pt[2]][recording_file_name]
                    feature_label_pairs = extract_features_segmented_class(signal=filtered_red_adc, 
                                                        labels_df=labels_df,
                                                        label_name=label_name,
                                                        pt_name = pt[2], 
                                                        fe_method=fe_method,
